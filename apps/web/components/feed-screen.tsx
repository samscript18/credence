...
export function FeedScreen() {
  ...
  const predictions = feed.data?.filter(
    (prediction) =>
      filter === "All" || prediction.visibility === (filter === "Public" ? "PUBLIC" : "LOCKED")
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="flex flex-col gap-5 rounded-xl border border-white/[0.04] bg-[#0B0C0E] p-5 transition-colors duration-200 hover:border-white/[0.08]">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="font-mono text-[28px] leading-none tracking-tight text-signal">
              {feed.isLoading ? "—" : activeCalls}
            </span>
        ...
        <div className="flex flex-col gap-5 rounded-xl border border-white/[0.04] bg-[#0B0C0E] p-5 transition-colors duration-200 hover:border-white/[0.08]">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="font-mono text-[28px] leading-none tracking-tight text-foreground">
              {feed.isLoading ? "—" : resolvedCalls}
            </span>
            <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-muted/70">
              verified
        ...
        <div className="flex flex-col gap-5 rounded-xl border border-white/[0.04] bg-[#0B0C0E] p-5 transition-colors duration-200 hover:border-white/[0.08]">
          ...
          <div className="flex items-baseline gap-1.5">
            <span className="font-mono text-[28px] leading-none tracking-tight text-emerald-400">
              15m
            </span>
            <span className="font-mono text-[11px] uppercase tracking-[0.16em] text-muted/70">
              contracts
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between sm:gap-6">
            <div className="space-y-1.5">
              ...
              <p className="max-w-xl text-[13px] leading-relaxed text-muted">
                Every prediction backed by a DreamDEX position on Somnia Shannon Testnet.
              </p>
            </div>
        ...
        {feed.isError && (
          <div className="p-6">
            <ContentState
              ...
              description={
                feed.data
                  ? "You're seeing the last loaded predictions. Try refreshing before taking action."
                  : "The network feed is temporarily unavailable. Please try again."
              }
              action={
            ...
            <div className="col-span-full">
              <ContentState
                ...
                action={
                  <Link
                    href="/markets"
                    className="inline-flex min-h-10 items-center gap-2 rounded-full bg-signal px-5 text-xs font-semibold text-[#04131f]"
                  >
                    Explore Markets
      ...
      <p className="border-t border-white/5 pt-4 font-mono text-[11px] leading-relaxed text-muted/60">
        Active insight can be gated. Resolved performance is permanent and public. Seeded demo predictions are clearly labeled.
      </p>
    </div>
  );
}
