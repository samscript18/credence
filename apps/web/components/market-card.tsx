...
export function MarketCard({ market, selected, onSelect }: { market: DreamDexMarketQuote; selected: boolean; onSelect: () => void }) {
	...
	return (
		<div
			...
			className={`group flex flex-col justify-between rounded-xl border p-5 transition-all duration-200 ${selected ? "border-signal/40 bg-[#16181D]" : "border-white/5 bg-[#0B0C0E] hover:border-white/10"}`}
		>
			<div>
				{/* Header */}
			...
			<div className="mt-5 flex items-center justify-between gap-3 border-t border-white/5 pt-3.5">
				...
				<button
					...
					className={`inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 font-mono text-xs font-medium transition-all active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-40 ${
						selected ? "bg-signal text-[#04131f] font-semibold" : "border border-white/10 bg-white/[0.03] text-foreground hover:bg-white/[0.07]"
					}`}
				>
