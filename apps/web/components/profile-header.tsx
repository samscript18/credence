...
export function ProfileHeader({ user }: { user: PredictorProfile }) {
	...
	const isOwner = Boolean(auth.data?.walletAddress) && auth.data?.walletAddress.toLowerCase() === user.walletAddress.toLowerCase();
	...
	return (
		<>
			<div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#0B0C0E] p-6 sm:p-8">
					...
					<div className="min-w-0">
						<div className="flex items-center gap-4 mb-4">
							...
							<div className="relative group shrink-0">
								<PredictorAvatar address={user.walletAddress} name={user.displayName} avatarUrl={user.avatarUrl} className="size-16 text-lg" />
								{isOwner && (
									<button
										type="button"
							...
							<div>
								...
								<div className="mt-1 flex items-center gap-2 font-mono text-[11px] text-muted">
									...
									<button type="button" onClick={copyAddress} className="cursor-pointer text-muted hover:text-foreground transition-colors" aria-label="Copy address">
						...
						<p className="max-w-md text-[13px] leading-relaxed text-muted">Permanent prediction performance on DreamDEX Event Contracts. Every resolved call remains visible onchain.</p>
					</div>
				...
				<div className="relative mt-8 grid grid-cols-2 gap-4 border-t border-white/5 pt-6 sm:grid-cols-5 font-mono">
					<div>
						...
						<div className="mt-1 text-2xl font-medium tracking-tight text-foreground">{user.rank ? `#${user.rank}` : "—"}</div>
					</div>
					<div>
						<div className="text-[10px] uppercase tracking-[0.18em] text-muted">Accuracy</div>
						<div className="mt-1 text-2xl font-medium tracking-tight text-foreground tabular-nums">{user.accuracy.toFixed(1)}%</div>
					</div>
					<div>
						<div className="text-[10px] uppercase tracking-[0.18em] text-muted">Resolved</div>
						<div className="mt-1 text-2xl font-medium tracking-tight text-foreground tabular-nums">{user.resolvedPredictions}</div>
					</div>
					<div>
						<div className="text-[10px] uppercase tracking-[0.18em] text-muted">Correct</div>
						<div className="mt-1 text-2xl font-medium tracking-tight text-signal tabular-nums">{user.correctPredictions}</div>
					</div>
					<div className="col-span-2 sm:col-span-1">
						...
						<div className="mt-1 text-2xl font-medium tracking-tight tabular-nums">
							<span className={user.realizedPnl.startsWith("+") ? "text-emerald-400" : user.realizedPnl.startsWith("-") ? "text-rose-400" : "text-foreground"}>{user.realizedPnl}</span>
						</div>
					</div>
				</div>
				<section className="mt-5 border-t border-white/10 pt-4" aria-label="Settled performance">
					...
					{user.settledPnlByToken?.map((group) => (
						<p key={group.token} className="mt-2 text-sm">
							{group.pnl} {group.symbol} · {group.pendingClaims} pending claims
						</p>
					))}
			...
			{isOwner && <EditProfileDialog isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} user={user} />}
		</>
	);
}
